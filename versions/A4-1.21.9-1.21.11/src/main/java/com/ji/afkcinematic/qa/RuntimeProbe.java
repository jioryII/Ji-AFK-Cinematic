...
public final class RuntimeProbe {
    ...
    private static final List<String> CRITICAL_MIXINS = List.of(
            "InGameHudMixin", "KeyboardMixin", "MouseMixin", "MinecraftClientMixin");
    ...
    private static void tick(MinecraftClient client) {
        ...
        if (phase == 0) {
            for (String mixin : CRITICAL_MIXINS) {
                check(MixinState.didApply(mixin), "critical mixin did not apply: " + mixin);
            }
            String cameraMixin = net.fabricmc.loader.api.FabricLoader.getInstance().getRawGameVersion()
                    .startsWith("1.21.11") ? "Camera211Mixin" : "CameraMixin";
            check(MixinState.didApply(cameraMixin), "critical mixin did not apply: " + cameraMixin);
            ...
            check(CinematicManager.getState() == CinematicState.CINEMATIC_ACTIVE,
                    "cinematic did not enter active state");
        ...
        if (phase == 1) {
            check(client.currentScreen instanceof ChatScreen, "chat screen did not remain open for a frame");
            check(CinematicManager.getState() == CinematicState.CINEMATIC_ACTIVE,
        ...
        if (phase == 2) {
            check(client.currentScreen instanceof ConfigScreen, "configuration screen did not remain open for a frame");
