...
public final class ComposedCharacterShot extends AbstractCameraShot {
    ...
    private float angle(float phase) {
        return startAngle + preset.sweep * preset.direction * phase;
    }
    ...
    public Vec3d updatePosition(float elapsedSeconds, float speedMultiplier, float tickDelta) {
        ...
        Vec3d offset = getCircularOffset(angle(travel), lerp(p, preset.startDistance, preset.endDistance));
