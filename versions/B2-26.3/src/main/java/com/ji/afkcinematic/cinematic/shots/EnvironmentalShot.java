...
public final class EnvironmentalShot extends AbstractCameraShot {
    ...
    public Vec3 updatePosition(float elapsedSeconds, float speedMultiplier, float tickDelta) {
        ...
        Vec3 offset = getCircularOffset(angle(travel),
    ...
    public float updateYaw(float elapsedSeconds, float speedMultiplier, float tickDelta) {
        float travel = travel(elapsedSeconds, speedMultiplier);
        float p = Math.min(1.0f, travel);
        // Orbit direction changes camera travel, not which way the lens points.
        // Reversing the yaw offset aimed several "environment" shots back at the player.
        return angle(travel) + lerp(p, preset.startYawOffset, preset.endYawOffset);
    }
