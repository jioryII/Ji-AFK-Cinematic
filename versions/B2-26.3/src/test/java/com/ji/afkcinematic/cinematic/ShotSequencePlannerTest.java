...
class ShotSequencePlannerTest {
    @Test
    void zeroAndOneHundredPercentNeverLeakTheOtherCategory() {
        for (long seed = 0; seed < 100; seed++) {
            for (int shot : ShotSequencePlanner.plan(0, 15, 15, seed)) assertTrue(shot >= 15);
            for (int shot : ShotSequencePlanner.plan(100, 15, 15, seed)) assertTrue(shot < 15);
        }
    }
