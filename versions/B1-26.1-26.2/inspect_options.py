...
def get_class_methods(class_path):
    ...
    for _ in range(fields_count):
        ...
        name = cp[name_idx]
        desc = cp[desc_idx]
        if 'smooth' in name.lower() or 'camera' in name.lower() or 'Lnet/minecraft/client/OptionInstance;' in desc:
            print(f"Field: {name} {desc} (Access: {hex(acc)})")
        pos += 6
    ...
    for _ in range(methods_count):
        ...
        desc = cp[desc_idx]
        if 'smooth' in name.lower() or 'camera' in name.lower() or '()Lnet/minecraft/client/OptionInstance;' in desc:
            print(f"Method: {name}{desc} (Access: {hex(acc)})")
        pos += 6
        attr_count = struct.unpack('>H', data[pos:pos+2])[0]
        pos += 2
        for _ in range(attr_count):
...
get_class_methods('net/minecraft/client/Options.class')
